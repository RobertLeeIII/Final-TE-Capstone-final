<template>
    ...
    <uninvite-from-potluck v-else-if="this.$route.query.action == 'uninvite'" :guests="potluckGuests"
        :potluck="currentPotluck"></uninvite-from-potluck>
    <div v-else>
        <div v-for="guest in potluckGuests" :key="guest.username"
            :class="guest.userId == currentPotluck.hostId ? 'isHost' : ''">{{ guest.username.substring(0, 1).toUpperCase() +
                guest.username.substring(1) }}</div>
